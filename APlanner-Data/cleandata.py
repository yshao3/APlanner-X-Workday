...
def importcourse(db,data):
	...
	for d in data:
		...
		data1.append([subject+" "+number,title,description,",".join(when_offer),",".join(tags),";".join(prerequisite), ";".join(notcur),",".join(tracks),str(min_unit)])
...
reader = csv.reader(open("CUCSFA17-cleaned (with track).csv","r"))
reader.next()
data = []
for row in reader:
	data.append(row)
